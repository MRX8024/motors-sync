...
class EncoderHelper(BaseSensorHelper):
    ...
    def _check_encoder_place(self):
        ...
        axis_steppers_n = [s.get_name() for s in sts['mcu_steppers']]
        if binded_stepper not in axis_steppers_n:
            raise self.printer.config_error(
                f"motors_sync: Encoder '{self.chip_name}' stepper "
                f"'{binded_stepper}' not in '{self.axis.name}' "
                f"axis steppers: '{', '.join(axis_steppers_n)}'")
...
class MotionAxis:
    ...
    def add_paired_axes(self, axes):
        for axis in axes:
            if axis is self:
                continue
            if axis not in self.paired_axes:
                self.paired_axes.append(axis)
                axis.add_paired_axes([self])
    ...
    def add_steppers(self, enable, step, buzz):
    ...
    def swap_steppers(self, en_stepper_name):
        ...
        if self.is_multi_axis:
            return False
        new_st = [st for st in sts['mcu_steppers']
    ...
    def get_phases_offset(self):
        if self.is_multi_axis:
            return None
        it = zip(self.steppers['mcu_steppers'], self.steppers['tmcs'])
        p1, p2 = (self._query_phase(s, t) for s, t in it)
        return p2 - p1
    ...
    def toggle_steppers(self, mode):
        mcu_steppers = self.steppers['mcu_steppers']
        ret = self.stepper_move.steppers_enable(mcu_steppers, mode)
        if ret:
            self.toolhead.dwell(MOTOR_STALL_TIME)

    def _toggle_not_steppers(self, mcu_steppers, mode):
        _mcu_steppers = []
        for mcu_stepper in self.steppers['mcu_steppers']:
            if mcu_stepper in mcu_steppers:
                continue
            _mcu_steppers.append(mcu_stepper)
        return self.stepper_move.steppers_enable(_mcu_steppers, mode)

    def toggle_paired_axes_steppers(self, mode):
        if not self.paired_axes:
            return
        did_change = False
        self_mcu_steppers = self.steppers['mcu_steppers']
        for axis in self.paired_axes:
            ret = axis._toggle_not_steppers(self_mcu_steppers, mode)
            did_change |= ret
        if did_change:
            self.toolhead.dwell(MOTOR_STALL_TIME)
